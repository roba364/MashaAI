...
final class RealmStorage: DatabaseStorage {
    ...
    func writeTransaction<Model>(
        ...
        try await executor.write { [triggers] in
            let result = try block(RealmContext(realm: $0,
                                                triggers: triggers))
...
private class RealmContext: DatabaseContext {
    ...
    private func collection<T: DBEntity>(_ type: T.Type,
                                         query: DBQuery<T>?,
                                         sort: SortParams? = nil) -> Results<T> {
        ...
        if let sort = sort {
            collection = collection.sorted(byKeyPath: sort.keyPath,
