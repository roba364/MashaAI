...
extension DatabaseStorage {
  func get<T: DBEntity, V: Comparable, Model>(
    _ type: T.Type = T.self,
    query: DBQuery<T>? = nil,
    sort: SortParams<T, V>? = nil,
...
extension DatabaseWrite {
  ...
  func delete<T: DBEntityWithID>(_ type: T.Type, by id: T.Identifier)
  where T.Identifier: Equatable {
    delete(
      type,
...
extension DatabaseWrite where Self: DatabaseRead {
  func updateIfExists<T: DBEntityWithID>(
    _ type: T.Type = T.self,
    by id: T.Identifier,
    modify: (T) -> Void
  ) {
